...
- **요약 섹션이 최우선이다.** 첫 문단만 읽어도 원래 요청·왜 중요했는지·결과/후속이 복원되어야 한다. 여기는 한 문단(3-6문장)을 써도 좋다. 1줄 압축 규칙은 요약 섹션에 적용하지 않는다.
- **나머지 항목은 1줄로.** 주요 결정·피드백·작업 내역·변경 사항은 장황하면 가치가 떨어진다.
- **빈 섹션은 생략한다.** 코드 변경이 없었으면 "변경 사항" 섹션을 넣지 않는다. 후속 작업이 없으면 해당 섹션도 생략.
- **한국어로 작성한다.** 기술 용어, 파일명, 코드는 원문 유지.
- **분량은 30줄 안팎을 목표로 하되 하드 제한은 아니다.** 줄이려면 "작업 내역"과 "변경 사항"부터 양보한다. 우선순위는 **요약 > 주요 결정 > 피드백 > 작업 내역 ≈ 변경 사항**. 핵심 결정을 잘라 30줄을 맞추지 말 것.
...
- 30줄은 목표지 하드 제한이 아니다. 분량을 맞추려다 요약·주요 결정·피드백이 잘려나가면 본말전도. 줄여야 하면 "작업 내역"과 "변경 사항"부터 양보하고, 요약 첫 문단의 복원력(원래 요청·맥락·결과)은 절대 희생하지 않는다.
